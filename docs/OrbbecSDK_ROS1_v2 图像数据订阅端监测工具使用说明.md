...
* 订阅端 CSV 中，绝对时间字段和时间差字段统一输出为 `_sec`，保留 6 位小数；脚本内部仍使用整数微秒计算，避免浮点累计误差。
...
| `ros_header_stamp_delta_sec` | 相邻两帧 ROS header 时间戳差，单位为秒，保留 6 位小数 | 判断消息源头是否停顿、回退或跳变 |
...
| `receive_steady_delta_sec` | 相邻两帧接收 steady clock 时间差，单位为秒，保留 6 位小数 | 判断 ROS 接收侧是否短暂停流或卡顿 |

system clock 可能受到手动校时或 NTP 校时影响，因此停流告警使用 `receive_steady_delta_sec`，不使用 `receive_system_ts_sec` 计算时间差。
...
| `receive_steady_delta_min_sec` | 最小接收 steady clock 时间差，单位为秒，保留 6 位小数 |
| `receive_steady_delta_max_sec` | 最大接收 steady clock 时间差，单位为秒，保留 6 位小数 |
| `receive_steady_delta_average_sec` | 平均接收 steady clock 时间差，单位为秒，保留 6 位小数 |
...
| `ros_header_stamp_delta_min_sec` | 最小 ROS header 时间戳差，单位为秒，保留 6 位小数，保留负值和零值 |
| `ros_header_stamp_delta_max_sec` | 最大 ROS header 时间戳差，单位为秒，保留 6 位小数 |
| `ros_header_stamp_delta_average_sec` | ROS header 时间戳正向时间差平均值，单位为秒，保留 6 位小数，排除零和负值 |
...
最小值、最大值和平均值均按秒输出并保留 6 位小数。如果某个话题尚未产生可统计时间差，次数字段为 `0`，其余统计值和最大时间差定位信息留空。如果 ROS header 时间戳差全部小于或等于零，`ros_header_stamp_delta_average_sec` 留空。
...
正常情况下，CSV 中的以下字段应大致在 `0.033333` 秒附近波动：
...
如果是 15 FPS，理论帧间隔约为 `0.066667` 秒。

### 5.2 短暂停流判断

如果 `receive_steady_delta_sec` 明显超过正常帧间隔，例如达到 `0.500000`、`1.000000` 或 `2.000000` 秒，说明 ROS 订阅端在这两帧之间没有收到该话题的新图像。

默认告警规则：

```text
receive_steady_delta_sec > 1.000000
或
距离上一帧的时间 > 1.000000 秒
```
...
```text
Image receive delta exceeded 1.000s: topic=/camera_01/color/image_raw, seq=12345, receive_steady_delta_sec=1.234567, ros_header_stamp_delta_sec=1.233333
```
...
```text
Image topic no frame for more than 1.000s: topic=/camera_01/color/image_raw, no_frame_duration_sec=1.234567, last_header_seq=12345, last_receive_system_ts_sec=1780541164.123456
```
...
* `receive_steady_delta_max_sec`：快速判断是否出现过较长接收时间差。
...
| 现象 | 可能含义 |
| --- | --- |
| `receive_steady_delta_sec` 和 `ros_header_stamp_delta_sec` 同时增大 | 发布端、设备端或上游链路可能停顿 |
| 仅 `receive_steady_delta_sec` 明显增大 | ROS 传输、订阅端调度或主机负载可能异常，需结合发布端监测工具分析 |
| `ros_header_stamp_delta_sec` 为负数 | ROS header 时间戳发生回退 |
